...
(define (rating-filter bitarray cmp)
...
(define (main args)
  (let ((one-false-array (list->array 2 (file->bitlist "input.txt")))) ; get the input convert to 2D array of 1s and #fs)
    ;; Part 1
    (let* ((t-one-false-array (transpose-array one-false-array 1 0))
    ...
    ;; Part 2
    (let ((oxygen (rating-filter one-false-array >=))
          (co2 (rating-filter one-false-array <)))
